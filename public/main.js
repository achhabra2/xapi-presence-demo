...
(function () {
	...
	socket.on('codecs', function (codecs) {
		...
		console.log(codecs);
		//console.log(placesByIp);
		for (var codec in codecs) {
			if (placesByIp[codecs[codec].ip]) {
				addTooltipOnPlace(`People Count: ${codecs[codec].lastCount}`, placesByIp[codecs[codec].ip], map);
				//setTooltipContentOnPlace('Count: ' + codecs[codec].lastCount, placesByIp[codecs[codec].ip], map);
			}
		}
	});
	...
	socket.on('PeopleCount', function (payload) {
		...
		//console.log(payload);
		if (placesByIp[payload.ip] && placeLayerByPlaceName[placesByIp[payload.ip].name])
			setTooltipContentOnPlace(`People Count: ${payload.data}`, placesByIp[payload.ip], map);
		else if (placesByIp[payload.ip]) {
			addTooltipOnPlace(`People Count: ${payload.data}`, placesByIp[payload.ip], map);
		}
	});
	...
	function addTooltipOnPlace(text, place, map) {
		...
		placeLayer.bindTooltip(text, { direction: 'top', opacity: 1, permanent: place.data.permanent, sticky: place.data.sticky, interactive: false, className: 'tooltip', pane: 'tooltipPane'}).addTo(map);
	...
	Mapwize.Api.getVenue(VENUEID, function (err, venue) {
		...
		Mapwize.Api.getPlaces({ venueId: VENUEID }, function (err, places) {
			...
			for (var place of places) {
				...
				// map.setPlaceStyle(place._id, style);
			}

			//Move to when socket receives codecs object
			// for (var place of places) {
			//   addTooltipOnPlace('Count:', place, map);
			// }

			//Only connect socket after all mapwize places received
